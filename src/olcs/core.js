...
export class olcsCore {
	 ...
	 * @api
	 */
	static computePixelSizeAtCoordinate(scene, target) {
	 ...
	 */
	static computeBoundingBoxAtTarget(scene, target, amount) {
	  const pixelSize = this.computePixelSizeAtCoordinate(scene, target);
	 ...
	 * @api
	 */
	static applyHeightOffsetToGeometry(geometry, height) {
	 ...
	 * @api
	 */
	static createMatrixAtCoordinates(coordinates, rotation = 0, translation = Cesium.Cartesian3.ZERO, scale = new Cesium.Cartesian3(1, 1, 1)) {
	  const position = this.ol4326CoordinateToCesiumCartesian(coordinates);
	 ...
	 * @api
	 */
	static rotateAroundAxis(camera, angle, axis, transform,
		opt_options) {
	 ...
	 * @api
	 */
	static setHeadingUsingBottomCenter(scene, heading,
		bottomCenter, opt_options) {
	  const camera = scene.camera;
	  // Compute the camera position to zenith quaternion
	  const angleToZenith = this.computeAngleToZenith(scene, bottomCenter);
	  ...
	  const rotateAroundAxis = this.rotateAroundAxis;
	 ...
	 * @api
	 */
	static pickOnTerrainOrEllipsoid(scene, pixel) {
	 ...
	 * @api
	 */
	static pickBottomPoint(scene) {
	  const canvas = scene.canvas;
	  const bottom = new Cesium.Cartesian2(
		  canvas.clientWidth / 2, canvas.clientHeight);
	  return this.pickOnTerrainOrEllipsoid(scene, bottom);
	}
	 ...
	 * @api
	 */
	static computeSignedTiltAngleOnGlobe(scene) {
	  const camera = scene.camera;
	  ...
	  const angleBetween = this.signedAngleBetween;
	 ...
	 * @return {number}
	 */
	static signedAngleBetween(first, second, normal) {
	 ...
	 * @api
	 */
	static computeAngleToZenith(scene, pivot) {
	  ...
	  const ray = this.bottomFovRay(scene);
	  ...
	  const a = this.signedAngleBetween(normal, direction, left);
	  return a + fy;
	}
	 ...
	 * @api
	 */
	static extentToRectangle(extent, projection) {
	 ...
	 */
	static sourceToImageryProvider(olMap, source, viewProj, olLayer) {
	  ...
	  if (source instanceof olSourceTileImage) {
		...
		if (this.isCesiumProjection(projection)) {
		...
		if (skip === false) {
		  ...
		  const rectangle = this.extentToRectangle(extent, projection);
	 ...
	 * @api
	 */
	static tileLayerToImageryLayer(olMap, olLayer, viewProj) {
	  ...
	  if (ext) {
		layerOptions.rectangle = this.extentToRectangle(ext, viewProj);
	  }
	 ...
	 * @api
	 */
	static updateCesiumLayerProperties(olLayerWithParents, csLayer) {
	  let opacity = 1;
	  let visible = true;
	 ...
	 * @api
	 */
	static ol4326CoordinateToCesiumCartesian(coordinate) {
	 ...
	 * @api
	 */
	static ol4326CoordinateArrayToCsCartesians(coordinates) {
	  console.assert(coordinates !== null);
	  const toCartesian = this.ol4326CoordinateToCesiumCartesian;
	  const cartesians = [];
	 ...
	 * @api
	 */
	static olGeometryCloneTo4326(geometry, projection) {
	 ...
	 * @api
	 */
	static convertColorToCesium(olColor) {
	 ...
	 * @api
	 */
	static resetToNorthZenith(map, scene) {
	  return new Promise((resolve, reject) => {
		const camera = scene.camera;
		const pivot = this.pickBottomPoint(scene);
		if (!pivot) {
		...
		const angle = this.computeAngleToZenith(scene, pivot);

		// Point to North
		this.setHeadingUsingBottomCenter(scene, currentHeading, pivot);
		...
		this.rotateAroundAxis(camera, -angle, axis, transform, options);
	  });
	}
	 ...
	 * @api
	 */
	static rotateAroundBottomCenter(scene, angle) {
	  return new Promise((resolve, reject) => {
		const camera = scene.camera;
		const pivot = this.pickBottomPoint(scene);
		if (!pivot) {
	 ...
	 * @api
	 */
	static normalizeView(view, angle = 0) {
	 ...
	 */
	static isCesiumProjection(projection) {
